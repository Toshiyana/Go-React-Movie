...
function OneGenreFunc(props) {
    // "movies" can't use "const", use "let"
    // the list of movies will be changed depending on which genre is clicked
    let [movies, setMovies] = useState([]);
    const [error, setError] = useState(null);
    let [genreName, setGenreName] = useState("");

    useEffect(() => {
        fetch("http://localhost:8080/v1/movies/" + props.match.params.id)
            .then((response) => {
